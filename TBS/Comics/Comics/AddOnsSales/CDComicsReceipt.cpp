...
DBTComicsToBeDelivered::DBTComicsToBeDelivered
	...
	:
	DBTSlaveBuffered (pClass, pDocument, _NS_DBT("ComicsToBeDelivered"), ALLOW_EMPTY_BODY, FALSE)
{
}
...
BEGIN_MESSAGE_MAP(CDComicsReceipt, CClientDoc)
	...
	ON_COMMAND(IDC_RECEIPT_FILTER_SEARCH, OnResearchComics)
...
{   
	...
	CreateSlaveView(IDD_BOX_SELECTION_RECEIPT_FILTER, NULL, 1);
}
...
{
	//CDComicsReceipt* pClientDoc = (CDComicsReceipt*)(GetDocument()->GetClientDoc(RUNTIME_CLASS(CDComicsReceipt)));

	//if (pClientDoc->CheckFilterBoxNo())
	//	pClientDoc->OpenComicsToBeDeliveredView();

	//CSlaveFormView* pView = (CSlaveFormView*)GetDocument()->CreateSlaveView(RUNTIME_CLASS(CComicsToBeDeliveredSlaveView), _TB("Ricercatore Fumetti da Consegnare"));
	CreateSlaveView(IDD_BOX_SELECTION_RECEIPT, NULL, 1);

	/*ASSERT_VALID(pView);
	if (!pView)
	{
		TRACE("fails to CreateSlaveView");
		return;
	}
	CSlaveFrame* pFrame = pView->GetFrame();
	if (pFrame)
		pFrame->SetOwner(GetDocument()->GetMasterFrame());
...
//-----------------------------------------------------------------------------
void CDComicsReceipt::OnEnd()
{
	CSlaveFormView* pView = (CSlaveFormView*)GetServerDoc()->ViewAlreadyPresent(IDD_BOX_SELECTION_RECEIPT);
	if (pView)
